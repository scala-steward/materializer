...
abstract class ZCaseApp[T](implicit val parser0: Parser[T], val messages: Help[T]) extends ZIOAppDefault {
  ...
  def run(options: T, remainingArgs: RemainingArgs): ZIO[Environment with Scope, Nothing, ExitCode]
...
abstract class ZCommandAppWithPreCommand[D, T](implicit
  ...
  /**
    ...
    */
  def beforeCommand(options: D, remainingArgs: Seq[String]): ZIO[Environment, IOException, Option[ExitCode]]
  ...
  def helpAsked(): ZIO[Environment, IOException, ExitCode] =
    printLine(
  ...
  def commandHelpAsked(command: Seq[String]): ZIO[Environment, IOException, ExitCode] =
    printLine(commandsMessages.messagesMap(command).helpMessage(beforeCommandMessages.progName, command))
      .as(ExitCode.success)

  def usageAsked(): ZIO[Environment, IOException, ExitCode] =
    printLine(
  ...
  def commandUsageAsked(command: Seq[String]): ZIO[Environment, IOException, ExitCode] =
    printLine(commandsMessages.messagesMap(command).usageMessage(beforeCommandMessages.progName, command))
      .as(ExitCode.success)
...
abstract class ZCommandApp[T](implicit
  ...
  override def beforeCommand(options: None.type, remainingArgs: Seq[String]): ZIO[Environment, IOException, Option[ExitCode]] = {
